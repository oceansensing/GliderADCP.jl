# End-to-end GliderADCP.jl example: sea064 M59 (NESMA Passengers, Jul 2024).
#
# Exercises the full stack on a third mission (subtropical, deep water, 2024-era
# GLIMPSE export): native .ad2cp binary — cross-checked against the MIDAS netCDF
# that also exists here — + multi-route SeaExplorer loading (glider-computer
# files and the GLIMPSE-server export merged and deduplicated) → QC →
# declination → ENU pings → shear-bias calibration → DAC (+ screened bottom
# track) → inverse & shear & vertical solutions → figures.
#
#   JULIA_LOAD_PATH="@:@ocean:@stdlib" julia +1.13 --project=. examples/m59_currents.jl

using GliderADCP
using DataFrames, Dates, Statistics, NaNStatistics
using Printf

const MISSION = "/Users/gong/oceansensing Dropbox/C2PO/glider/gliderData/sea064-20240720-nesma-passengers-complete"
const OUT = joinpath(@__DIR__, "output")
mkpath(OUT)

@info "1/6 load AD2CP (native binary) + navigation (delayed + GLIMPSE, deduplicated)"
adcp = read_ad2cp(joinpath(MISSION, "ad2cp/sea064_M59.ad2cp"))
@info "    $adcp"
# this mission also has the MIDAS netCDF export — verify reader parity
let nc = load_ad2cp(joinpath(MISSION, "ad2cp/sea064_M59.ad2cp.00000.nc"))
    @assert length(nc) == length(adcp)
    dv = maximum(abs, filter(!isnan, vec(nc.vel .- adcp.vel)))
    @info "    binary vs MIDAS netCDF: $(length(nc)) ensembles, max |Δvel| = $dv"
end
cov = coverage(adcp)
...
nav = load_seaexplorer_nav([joinpath(MISSION, "delayed/nav/logs"),
                            joinpath(MISSION, "glimpse")]; stream="59.gli.sub")
...
pld = load_seaexplorer_pld([joinpath(MISSION, "delayed/pld1/logs"),
                            joinpath(MISSION, "glimpse")]; stream="59.pld1.sub")
...
save(joinpath(OUT, "M59_diagnostics.png"), fig2)

println("\nM59 outputs in $(OUT):")
foreach(f -> startswith(f, "M59") && println("  ", f), readdir(OUT))
